...
app.use(cors());
app.use(express.static(path.join(__dirname, 'MediaMandrill')));

app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'MediaMandrill', 'index.html'));
});
...
// Toutes les requêtes en attente — clé : requestId unique
...
const wss = new WebSocketServer({ noServer: true }); // serveur WebSocket, écoute sur le port 4081
...
restServer.listen(4080, '0.0.0.0', () => { console.log('[Server] REST API  → http://0.0.0.0:4080'); });
if (restServerSSL) {
	restServerSSL.listen(4443, '0.0.0.0', () => { console.log('[Server] REST API  → https://x1tablet.local:4443'); });
}

wsServer.listen(4081, '0.0.0.0', () => { console.log('[Server] WebSocket → ws://0.0.0.0:4081/ws'); });
if (wsServerSSL) {
	wsServerSSL.listen(4444, '0.0.0.0', () => { console.log('[Server] WebSocket → wss://x1tablet.local:4444/ws'); });
}


// ---------------------------
// WebSocket — MediaMonkey
// ---------------------------
